...
class CmtHead(BaseModule):
    ...
    def _get_targets_single(self, gt_bboxes_3d, gt_labels_3d, pred_bboxes, pred_logits):
        ...
        def task_assign(bbox_pred, logits_pred, gt_bboxes, gt_labels, num_classes):
            ...
            sampling_result = self.sampler.sample(assign_results, InstanceData(priors=bbox_pred), InstanceData(bboxes_3d=gt_bboxes))
    ...
    def _dn_loss_single_task(self,
        ...
        map_known_indice = mask_dict['map_known_indice'].long().to(pred_logits.device)
        known_indice = mask_dict['known_indice'].long().to(pred_logits.device)
        batch_idx = mask_dict['batch_idx'].long().to(pred_logits.device)
